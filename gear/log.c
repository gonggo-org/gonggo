...
void log_context_init(LogContext *ctx, pid_t pid, const char *path, const char *gonggo_name,  pthread_mutex_t *mtx) {
    if(ctx==NULL)
        return;
    ctx->pid = pid;
    ctx->path = path;
...
void log_context_destroy(LogContext *ctx) {
    if( ctx!=NULL && ctx->mtx!=NULL ) {
        pthread_mutex_destroy(ctx->mtx);
        ctx->mtx = NULL;
    }
}

void gonggo_log(const LogContext *ctx, const char *level, const char *fmt, ...) {
    time_t t;
    ...
    sprintf(filepath, "%s/%s-%s.log", ctx->path, ctx->gonggo_name, tmstr);
