...
bool alive_mutex_create(bool alive) {
    ...
    asprintf(&alive_mutex_path, "/%s_alive", gonggo_name);
    ...
    if(fd==-1) {
        ...
        gonggo_log("ERROR", "shm %s creation failed, %s", alive_mutex_path, buff);
    ...
    if(alive_mutex_shm==MAP_FAILED) {
        strerror_r(errno, buff, GONGGOLOGBUFLEN);
        gonggo_log("ERROR", "shm %s map failed, %s", alive_mutex_path, buff);
        close(fd);
